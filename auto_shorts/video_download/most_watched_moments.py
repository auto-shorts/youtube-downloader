...
class MostReplayedNotPresentException(Exception):
    ...
    def __init__(
        self, message="Video doesn't have most replayed moments avaible"
    ):
...
class MostWatchedMomentsDownloader(MostWatchedMomentsDownloaderBase):
    ...
    def _preprocess_results(self, raw_results: dict) -> pd.DataFrame:
        cleaned_timeframes = []
        ...
        for timeframe in raw_results["items"][0]["mostReplayed"][
            ...
            one_timeframe_cleaned["time_end_ms"] = (
                timeframe_data["timeRangeStartMillis"]
                + timeframe_data["markerDurationMillis"]
            )
            one_timeframe_cleaned["peroid_duration_ms"] = timeframe_data[
                "markerDurationMillis"
            ]
            one_timeframe_cleaned["intensity_score"] = timeframe_data[
